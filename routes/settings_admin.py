...
def wipe_accounts():
    ...
    cur.execute("SELECT password FROM users WHERE id = %s", (session['user_id'],))
    result = cur.fetchone()
    if not result or not check_password_hash(result[0], password_input):
        cur.close()
...
@admin_bp.route('/wipe/app-users', endpoint='wipe_app_users')
def wipe_app_users():
    if 'user_id' not in session or not session.get('is_admin'):
        return redirect('/')
    cur = mysql.connection.cursor()
...
@admin_bp.route('/wipe/servers', endpoint='wipe_servers')
def wipe_servers():
    if 'user_id' not in session or not session.get('is_admin'):
        return redirect('/')
    cur = mysql.connection.cursor()
...
@admin_bp.route('/wipe/apps', endpoint='wipe_apps')
def wipe_apps():
    if 'user_id' not in session or not session.get('is_admin'):
        return redirect('/')
    cur = mysql.connection.cursor()
...
@admin_bp.route('/wipe/all', endpoint='hard_reset')
def hard_reset():
    if 'user_id' not in session or not session.get('is_admin'):
        return redirect('/')
    cur = mysql.connection.cursor()
...
def view_logs():
    ...
    # 1. Učitaj logove
    cur.execute("""
        SELECT timestamp, actor, action, target_type, target_name, details
        FROM audit_logs
    ...
    # 2. Učitaj trenutnu vrednost za retention
    cur.execute("SELECT log_retention_days FROM settings LIMIT 1")
    result = cur.fetchone()
    cur.close()

    current_retention = int(result[0]) if result else 30  # fallback ako nije setovano
...
# CSV Imports
# ------------------------
@admin_bp.route('/import-users-csv', methods=['POST'], endpoint='import_users_csv')
def import_users_csv():
    return handle_csv_import(
        table='app_users',
        required_fields=['full_name', 'email', 'note', 'role'],  # Changed full_name -> name
        insert_sql="INSERT INTO app_users (full_name, email, note, role) VALUES (%s, %s, %s, %s)"  # same here
    )

@admin_bp.route('/import-servers-csv', methods=['POST'], endpoint='import_servers_csv')
...
@admin_bp.route('/import-applications-csv', methods=['POST'], endpoint='import_applications_csv')
...
def handle_csv_import(table, required_fields, insert_sql):
    ...
    try:
        ...
        flash(f'Error importing CSV to {table}: {e}', 'danger')
...
# ------------------------
@admin_bp.route('/download-users-template', endpoint='download_users_template')
def download_users_template():
    return generate_csv_template("name,email,note,role", "app_users_template.csv")

@admin_bp.route('/download-servers-template', endpoint='download_servers_template')
def download_servers_template():
    return generate_csv_template("name,ip_address,description", "servers_template.csv")

@admin_bp.route('/download-applications-template', endpoint='download_applications_template')
